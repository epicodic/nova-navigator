...
class Terminal(Widget, can_focus=True):
    ...
    def __init__(
        ...
        self.recv_queue_precmd: asyncio.Queue = None
        self.recv_precmd_task: Task = None
    ...
    def start(self) -> None:
        ...
        self.recv_queue_precmd = self.emulator.send_queue_precmd
        self.recv_task = asyncio.create_task(self.recv())
        self.recv_precmd_task = asyncio.create_task(self.recv_precmd())
    ...
    async def recv_precmd(self) -> None:
        try:
            while True:
                message = await self.recv_queue_precmd.get()
                logging.debug(f"recv_precmd: {message}")
...
class TerminalEmulator:
    def __init__(self, command: str):
        ...
        self.fd, self.fd_pre_cmd, self.fd_pre_cmd_child = self.open_terminal(command=command)
        self.p_out = os.fdopen(self.fd, "w+b", 0)  # 0: buffering off
        self.p_out_pre_comd = os.fdopen(self.fd_pre_cmd, "w+b", 0)  # 0: buffering off
        self.recv_queue = asyncio.Queue()
        self.send_queue = asyncio.Queue()
        self.send_queue_precmd = asyncio.Queue()
    ...
    async def _run(self) -> None:
        ...
        def on_pre_cmd() -> None:
            try:
                data = self.p_out_pre_comd.read(65536).decode()
                logging.debug(f"TerminalEmulator pre-cmd read {data} ")
                # self.event.set()
        ...
        loop.add_reader(self.p_out_pre_comd, on_pre_cmd)
