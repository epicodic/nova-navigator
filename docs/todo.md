...
Wastes CPU and adds up to 10 ms latency on every response response.

**Fix:**
- Change `GuiRequestCallback` type: replace `asyncio.Future[Response]` with `concurrent.futures.Future[Response]`
- In `_create_response_requester`: create `concurrent.futures.Future[Response]()` directly — no need to
...
**Problem:** `_responses_to_all` is a `dict[str, Response]` keyed on the dialog title string.
Any two dialogs with the same title accidentally share a cached response.
Currently harmless (copy and move both use `"Overwrite"` and mean the same thing), but easy to
break when adding new dialogs.

**Proposed fix:** Key on a dedicated `ResponseRequest` identity token rather than the human-readable
title. Options:
- Add an explicit `response_id: str` field to `ResponseRequest` (separate from display title)
- Or use the full `(title, expected_responses)` tuple as the key to at least reduce collision risk

---

## ~~5. `_POSITIVE = 0x0000` zero-valued Flag member — `response.py`~~
...
## 6. Global response serialization across parallel schedulers — `scheduler.py`

**Current behaviour:** `_request_lock` only serializes responses *within* a single
...
- `Dialog` is now `ModalScreen[Response | None]`; `ButtonSpec` carries a `response: Response` field
