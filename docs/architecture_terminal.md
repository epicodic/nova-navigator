...
| `stream` | `pyte.Stream` | pyte ANSI parser, feeds bytes into `_screen` |
| `_display` | `TerminalDisplay` | The last rendered frame, returned by `render()` |
| `send_queue` | `asyncio.Queue` | Commands from the widget to the PTY writer task |
| `recv_queue` | `asyncio.Queue` | Raw events from the PTY reader, consumed by `recv()` |
| `_run` task | `asyncio.Task` | PTY I/O pump — writes stdin, processes size changes |
| `recv_task_t` | `asyncio.Task` | Processes `recv_queue` entries and updates the display |
...
| `shell_cmd_cd(path)` | Returns a shell command to `cd` to `path` silently |
...
| `_TERMINAL_COLORS` | Maps the 8 named pyte terminal colors to hex values |
...
5. The PTY master fd is wrapped in a buffered `io.FileIO` (`self.p_out`).
6. The pre-cmd pipe read end is wrapped in `self.p_out_pre_cmd`.
...
asyncio event loop
│
├── loop.add_reader(p_out,         on_output)   ← fires when PTY master has data
├── loop.add_reader(p_out_pre_cmd, on_pre_cmd)  ← fires when shell writes pwd
│
├── Task: _run()      ← reads send_queue, writes stdin / resize to PTY
...
Awaits messages from `send_queue` and dispatches:

| Message | Action |
|---------|--------|
| `["stdin", text]` | Writes encoded bytes to `p_out` (the PTY master) |
| `["set_size", rows, cols]` | Sends `TIOCSWINSZ` ioctl to resize the PTY |
| `["click", x, y, btn]` | Writes an X10 mouse press/release escape sequence |
...
1. **ANSI scan:** The raw text is searched with `_re_ansi_sequence` for DECSET `?1000h`/`?1000l` sequences to update `mouse_tracking`.

2. **pyte feed:** `self.stream.feed(chars)` parses the ANSI escape sequences and updates the pyte `TerminalPyteScreen` buffer (cursor position, character cells, colors, attributes).
...
3. **Rich Text conversion:** For each row in the pyte buffer, a `rich.text.Text` object is built.
   ...
   When the style of character `x` differs from character `x-1`, `Text.stylize` is called to apply the previous style to the completed run.
   The last run is closed at `x == columns - 1`.
   Cursor position is applied as a `"reverse"` span inline.
...
5. **`refresh()`:** The widget is marked dirty; Textual calls `render()` which returns `self._display`; the Textual rendering pipeline calls `TerminalDisplay.__rich_console__` which applies the cursor `"reverse"` span and yields the lines to the console.
...
1. On `on_mount`, `MainScreen` calls `shell_init_code(fd_pre_cmd_child)` to generate:
   ```zsh
...
Mouse tracking is only active when the running application (e.g., `vim`, `htop`) sends a DECSET `?1000h` sequence.
The `recv()` loop scans all outgoing stdout for these sequences and flips `self.mouse_tracking`.
...
- A 6-digit lowercase hex string (e.g., `"ff6600"` for 256-color/truecolor)

`_translate_terminal_color` maps these to Rich-compatible color strings:
- Hex strings → `"#rrggbb"`
- Named colors → the hex values in `_TERMINAL_COLORS` (a fixed 8-color palette)
