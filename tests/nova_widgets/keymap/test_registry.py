...
class _MockContextResolver:
    """Minimal ContextResolver implementation for testing."""

    def __init__(self, context: str = "browser") -> None:
...
class _BrowserWidget(Widget):
    ACTIONS: ClassVar[list[Action]] = [
        Action("Copy", name="browser.copy", action="copy", contexts=["browser"], default_key="f5"),
        Action("Filter", name="browser.filter", action="filter", contexts=["browser"], default_key="ctrl+f"),
    ]
...
async def test_registry_collect_actions_from_widget_tree() -> None:
    resolver = _MockContextResolver("browser")
...
@pytest.mark.asyncio
async def test_registry_key_wrong_context_not_consumed() -> None:
    resolver = _MockContextResolver("terminal")
    registry = KeymapRegistry(resolver)
    registry.reload({"browser.copy": "f5"})
...
class _RealFocusableWidget(Widget, can_focus=True):
    ...
    ACTIONS: ClassVar[list[Action]] = [
        Action("Select", name="widget.select", action="select", contexts=["browser"], default_key="insert"),
    ]
...
class _RealDispatchScreen(Widget):
    ...
    ACTIONS: ClassVar[list[Action]] = [
        Action("Copy", name="screen.copy", action="copy_files", contexts=["browser"], default_key="f5"),
    ]
